...
export class ButtonDeleteService {
  ...
  // @todo: add confirm that someone wants to delete an item
  // @todo: make it so the page kinda refreshes on success so it doesn't show in array anymore
  initializeDelete(pageItem, pageName, itemIndex) {
    ...
    this.deleteRequest(pageItem, pageName)
        .subscribe(result => {
              if (result === false) {
                ...
                pageItem = null;
                this.pageInfoService.pageItems.slice(itemIndex);
              }
            }
        );
  }
